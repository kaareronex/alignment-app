...
- `ANTHROPIC_API_KEY` — Claude API key. **Server-only secret.** Used exclusively by `lib/ai/interview-model.ts`, which is the only file that talks to Anthropic directly — everything else calls `getNextInterviewTurn()`.
- `NEXT_PUBLIC_APP_URL` — this deployment's own base URL (no trailing slash), used to build the shareable interview link shown on `/admin/[projectId]` (see `app/admin/[projectId]/shareable-link-card.tsx`). Set to `http://localhost:3000` for local dev, and to the production domain (e.g. `https://alignment-app-blue.vercel.app`) in Vercel's project settings. Deliberately an explicit var rather than Vercel's `VERCEL_URL` - that one points at the unique per-deployment URL (a preview build's hash subdomain), not the stable domain leaders are actually sent.
